...
/**
 * Seed initial DocumentTypes if collection is empty
 */
const seedDocuments = async () => {
  try {
    const count = await DocumentType.countDocuments();
    if (count === 0) {
      await DocumentType.insertMany(documentTypes);
      logger.info('✅ 5 documents insérés en base');
    } else {
      logger.info('ℹ️ Documents déjà présents');
    }
  } catch (error) {
